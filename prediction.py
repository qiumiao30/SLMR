...
class Predictor:
    ...
    def get_score(self, values):
        ...
        with torch.no_grad():
            for x, _, y in tqdm(loader):
                ...
                y_hat, _ = self.model(x) # 预测值

                # Shifting input to include the observed value (y) when doing the reconstruction
                recon_x = torch.cat((x[:, 1:, :], y), dim=1) # 将预测值与去掉一行的原始数据拼接，进行重构，在MSL数据集上shape为[batch, win, feature]
                _, window_recon = self.model(recon_x) # model()返回两个值：预测值与重构值,只保留重构值
        ...
        recons = np.concatenate(recons, axis=0) # # 拼接重构值,MSL数据集shape为(train set shape-win, 1)
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        if true_anomalies is not None:
            bf_eval = bf_search(test_anomaly_scores, true_anomalies, start=0, end=0.5, step_num=3000, verbose=False)
        else:
            bf_eval = {}
        ...
        for i in range(out_dim):
            ...
            test_feature_anom_scores = test_pred_df[f"A_Score_{i}"].values
            # epsilon = find_epsilon(train_feature_anom_scores, reg_level=2)
            bf = bf_eval["threshold"]

            train_feature_anom_preds = (train_feature_anom_scores >= bf).astype(int)
            test_feature_anom_preds = (test_feature_anom_scores >= bf).astype(int)

            train_pred_df[f"A_Pred_{i}"] = train_feature_anom_preds
            test_pred_df[f"A_Pred_{i}"] = test_feature_anom_preds

            train_pred_df[f"Thresh_{i}"] = bf
            test_pred_df[f"Thresh_{i}"] = bf
        ...
        if save_output:
            ...
            train_pred_df[f"A_Pred_Global"] = (train_anomaly_scores >= global_bf).astype(int)
            test_preds_global = (test_anomaly_scores >= global_bf).astype(int)
