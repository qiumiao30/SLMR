...
def pot_eval(init_score, score, label, q=1e-3, level=0.99, dynamic=False, adjust_score=args.adjust_score):
    """
    ...
    if adjust_score:
        ...
        pred, p_latency = (score >= pot_th).astype(int), 0
...
def bf_search(score, label, start, end=None, step_num=1, display_freq=10, verbose=True):
    """
...
def epsilon_eval(train_scores, test_scores, test_labels, reg_level=1, adjust_score=args.adjust_score):
    ...
    if adjust_score:
        ...
        pred, p_latency = (test_scores>=best_epsilon).astype(int), 0
