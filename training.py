...
class Trainer:
    ...
    def fit(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            for x, x_mask, y in train_loader:
                x = x.to(self.device)
                x_mask = x_mask.to(self.device)
                y = y.to(self.device)
                self.optimizer.zero_grad()

                preds, recons = self.model(x_mask)
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for x, mask, y in data_loader:
                ...
                forecast_loss = torch.sqrt(self.forecast_criterion(y, preds)) # nn.MSE
                recon_loss = torch.sqrt(self.recon_criterion(x, recons)) # nn.MSE
        ...
        forecast_loss = np.sqrt((forecast_losses ** 2 + 1e-8).mean())
        recon_loss = np.sqrt((recon_losses ** 2 + 1e-8).mean())
